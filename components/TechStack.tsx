...
const TechStack = () => {
  return (
    <section className="py-16 md:py-32 px-4 md:px-6 max-w-7xl mx-auto flex flex-col items-center w-full overflow-x-hidden">
      <motion.div 
      ...
      <div className="w-full bg-white rounded-[2rem] md:rounded-[3rem] p-5 sm:p-8 md:p-16 shadow-xl relative overflow-hidden border border-gray-200">
        {/* Glow Effects */}
        ...
        >
          {tools.map((tool, index) => (
            <motion.div 
              ...
              key={index} 
              className={`flex flex-col items-center justify-center text-center p-4 md:p-8 rounded-2xl md:rounded-3xl border ${tool.border} bg-gray-50/80 backdrop-blur-md hover:bg-white transition-all duration-300 group hover:-translate-y-2 hover:shadow-xl min-w-0`}
            >
              ...
              <span className="text-xs sm:text-sm font-semibold text-gray-600 group-hover:text-gray-900 transition-colors tracking-wide text-center break-words">{tool.name}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
