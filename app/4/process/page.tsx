...
export default function ProcessPage() {
  return (
    <main className="min-h-screen bg-[#020202] text-gray-900 font-sans selection:bg-[#0B57D0] selection:text-white overflow-hidden">
      <Navbar />
      ...
      <div className="relative pt-40 pb-20 px-4 md:px-6 max-w-5xl mx-auto text-center">
        ...
        >
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 tracking-tight leading-[1.1]">
            How we ship in <span className="text-[#0B57D0]">30 Days.</span>
          </h1>
      ...
      <div className="px-4 md:px-6 max-w-4xl mx-auto pb-32 relative z-10">
        ...
        {/* Timeline Line */}
        <div className="absolute left-8 md:left-[50px] top-0 bottom-0 w-px bg-gray-800 hidden md:block"></div>
          ...
          {processSteps.map((step, index) => (
            ...
            >
              ...
              <div className="hidden md:flex flex-col items-center z-10">
                <div className="w-24 h-24 bg-[#0A0A0A] border border-gray-800 rounded-full flex items-center justify-center shadow-xl mb-4 relative group">
              ...
              {/* Card Content */}
              <div className="flex-1 bg-[#0A0A0A] border border-gray-800 rounded-[2rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group hover:border-gray-600 transition-colors duration-500">
                <div className="absolute top-0 right-0 text-[8rem] font-black text-white/[0.02] -translate-y-10 translate-x-10 pointer-events-none group-hover:scale-110 transition-transform duration-700">
                  {step.id}
                </div>
                
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                  <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{step.title}</h3>
                  <div className="px-4 py-1.5 rounded-full bg-[#111] border border-gray-800 text-gray-400 text-sm font-semibold whitespace-nowrap w-fit">
                    {step.duration}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 relative z-10">
                  {step.metrics.map((metric, i) => (
                    <div key={i} className="px-4 py-2 rounded-lg bg-[#111] border border-gray-800 text-gray-300 text-sm font-medium">
                      {metric}
                    </div>
                  ))}
                </div>
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-32 text-center bg-[#0A0A0A] border border-gray-800 rounded-[2rem] p-12 relative overflow-hidden"
        >
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-blue-600/10 blur-[100px] rounded-full pointer-events-none"></div>
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 relative z-10 tracking-tight">Ready to start the clock?</h2>
          <Link 
            href="/4/contact"
            className="inline-flex items-center gap-2 bg-white text-black px-8 py-4 rounded-full font-bold hover:bg-gray-200 transition-colors shadow-xl relative z-10"
          >
